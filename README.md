...
Pre-built binaries include the correct version number (e.g., `check-image version` returns `v0.11.1`).
...
go install github.com/jarfernandez/check-image/cmd/check-image@v0.11.1
```
...
```bash
docker pull ghcr.io/jarfernandez/check-image:0.11.1
```
...
```yaml
- uses: jarfernandez/check-image@v0.11.1
  with:
    image: nginx:latest
```
...
| `version` | No | `0.11.1` | check-image version to use |
