...
Pre-built binaries include the correct version number (e.g., `check-image version --short` returns `v0.20.0`). <!-- x-release-please-version -->
...
go install github.com/jarfernandez/check-image/cmd/check-image@v0.20.0 # x-release-please-version
```
...
```bash
docker pull ghcr.io/jarfernandez/check-image:0.20.0 # x-release-please-version
```
...
```yaml
- uses: jarfernandez/check-image@v0.20.0 # x-release-please-version
  with:
    image: nginx:latest
```
...
| `version` | No | `0.20.0` <!-- x-release-please-version --> | check-image version to use |
...
The project has comprehensive unit tests with 93.2% overall coverage. All tests are deterministic, fast, and run without requiring Docker daemon, registry access, or network connectivity.
...
- **internal/imageutil**: 86.3% coverage
- **cmd/check-image/commands**: 90.4% coverage
- **cmd/check-image**: 60.9% coverage
