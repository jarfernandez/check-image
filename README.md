...
Pre-built binaries include the correct version number (e.g., `check-image version --short` returns `v0.20.2`). <!-- x-release-please-version -->
...
go install github.com/jarfernandez/check-image/cmd/check-image@v0.20.2 # x-release-please-version
```
...
```bash
docker pull ghcr.io/jarfernandez/check-image:0.20.2 # x-release-please-version
```
...
```yaml
- uses: jarfernandez/check-image@v0.20.2 # x-release-please-version
  with:
    image: nginx:latest
```
...
| `version` | No | `0.20.2` <!-- x-release-please-version --> | check-image version to use |
